...
def do_eval(dataset, model, device, max_num_tokens, qc=None):
    model.eval()
    rand = 0
    nmi = 0
    num = 0
    ...
    for article_sample in dataset:
        for batch in article_sample:
            ...
            if qc is not None and batch.q in qc.keys():
                context = qc[batch.q]
...
def do_dkm_param_eval(dataset, model, max_num_tokens, qc=None):
    model.eval()
    rand = 0
    nmi = 0
    num = 0
    ...
    for article_sample in dataset:
        for batch in article_sample:
            ...
            if qc is not None and batch.q in qc.keys():
                context = qc[batch.q]
                query_content = ' '.join(context.split(' ')[:max_num_tokens // 2])
            pred_labels = model.get_clustering(query_content, input_texts, k)
            preds.append(pred_labels)
            rand += adjusted_rand_score(true_labels, pred_labels)
            nmi += normalized_mutual_info_score(true_labels, pred_labels)
            num += 1
...
def treccar_clustering_dkm_param_model(train_dataset,
    ...
    if query_context_ref is not None:
        val_rand, val_nmi = do_dkm_param_eval(val_dataset, model, max_num_tokens, qc)
        test_rand, test_nmi = do_dkm_param_eval(test_dataset, model, max_num_tokens, qc)
    else:
        val_rand, val_nmi = do_dkm_param_eval(val_dataset, model, max_num_tokens)
        test_rand, test_nmi = do_dkm_param_eval(test_dataset, model, max_num_tokens)
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        for idx in tqdm(range(len(train_ids))):
            ...
            for batch in article_sample:
                k = len(set(batch.para_labels))
                query_content = batch.q  # cat
                if query_context_ref is not None:
                    with open(query_context_ref, 'r') as f:
                        qc = json.load(f)
                    context = qc[batch.q]
                    query_content = ' '.join(context.split(' ')[:max_num_tokens // 2])
                mc, ma = model(query_content, batch.para_texts, k)
...
def treccar_clustering_single_model(train_dataset,
    ...
    schd = transformers.get_linear_schedule_with_warmup(opt, warmup, num_epochs * num_train_steps)
    if query_context_ref is not None:
        val_rand, val_nmi = do_eval(val_dataset, model, device, max_num_tokens, qc)
        test_rand, test_nmi = do_eval(test_dataset, model, device, max_num_tokens, qc)
    else:
        val_rand, val_nmi = do_eval(val_dataset, model, device, max_num_tokens)
        test_rand, test_nmi = do_eval(test_dataset, model, device, max_num_tokens)
    print('\nVal RAND %.4f, Val NMI %.4f, Test RAND %.4f, Test NMI %.4f' % (val_rand, val_nmi, test_rand, test_nmi))
    if loss_name == 'nmi':
        loss_func = get_nmi_loss
    else:
...
parser.add_argument('-vt', '--vital_cats', help='Path to vital article categories', default='/home/sk1105/QSC_data/vital_wiki/vital_article_cats_wo_people.json')
...
parser.add_argument('-qc', '--query_context', help='Path to query context file', default='/home/sk1105/QSC_data/vital_wiki/vital_article_cats_description.json')
...
if args.experiment == 1:
    treccar_clustering_single_model(train_dataset, test_dataset, device, val_dataset, args.query_context)
elif args.experiment == 2:
    treccar_clustering_dkm_param_model(train_dataset, test_dataset, device, val_dataset, args.query_context)
elif args.experiment == 3:
    treccar_clustering_baseline_sbert_triplet_model(train_dataset, test_dataset, device, val_dataset, args.query_context)