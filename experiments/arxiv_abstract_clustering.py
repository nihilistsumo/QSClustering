...
from core.clustering import QuerySpecificClusteringModel, SBERTTripletLossModel, get_nmi_loss, \
    get_weighted_adj_rand_loss, QuerySpecificDKM, QuerySpecificAttentionClusteringModel
...
def do_eval(test_data, model, max_num_tokens, qc=None, triplet_model=False):
    model.eval()
    rand_dict, nmi_dict = {}, {}
    test_data_subjects = list(test_data.keys())
    for si in tqdm(range(len(test_data_subjects))):
        s = test_data_subjects[si]
        test_abstracts = test_data[s]
        true_labels = [d['label'] for d in test_abstracts]
        k = len(set(true_labels))
        if triplet_model:
            input_texts = [d['abstract'] for d in test_abstracts]
            embeddings = model.emb_model.encode(input_texts, convert_to_tensor=True)
            pred_labels = model.get_clustering(embeddings, k)
        elif isinstance(model, QuerySpecificClusteringModel):
            texts = [d['abstract'] for d in test_abstracts]
            ...
            pred_labels = model.get_clustering(embeddings, k)
        elif isinstance(model, QuerySpecificAttentionClusteringModel):
            input_texts = [d['abstract'] for d in test_abstracts]
            embeddings = model.get_embedding_without_attn(input_texts)
            pred_labels = do_eval_attn_on_cpu(embeddings, k, model)
        else:
...
def do_eval_attn_on_cpu(embeddings, k, model):
    model.cpu()
    e = embeddings.cpu()
    pred_labels = model.get_clustering(e, k)
...
def arxiv_clustering_attn_model(arxiv_5cv_data_file,
                                    device,
                                    loss_name,
                                    max_num_tokens,
                                    max_grad_norm,
    ...
    for i in range(len(cv_datasets)):
        train_data_current = cv_datasets[i]
        test_data_current = {}
        for s in train_data_current.test_arxiv_data.keys():
            test_abstracts = train_data_current.test_arxiv_data[s]
            if len(test_abstracts) > 10000:
                test_abstracts = random.sample(test_abstracts, 10000)
            test_data_current[s] = test_abstracts
        num_steps_per_epoch = len(train_data_current)
        num_train_steps = num_epochs * num_steps_per_epoch
        model = QuerySpecificAttentionClusteringModel(emb_model_name, emb_dim, attn_dim, num_attn_head, device,
                                                      max_num_tokens).to(device)
        model_params = list(model.named_parameters())
        no_decay = ['bias', 'LayerNorm.bias', 'LayerNorm.weight']
        optimizer_grouped_parameters = [
            {'params': [p for n, p in model_params if not any(nd in n for nd in no_decay)],
        ...
        for epoch in tqdm(range(num_epochs)):
            ...
            for idx in tqdm(range(len(train_data_current))):
                ...
                n = len(sample.papers)
                k = len(set(sample.paper_labels))
                input_texts = sample.paper_texts
                input_features = model.qp_model.tokenize(input_texts)
                put_features_in_device(input_features, device)
                #print(GPUtil.showUtilization())
                mc, ma = model(input_features, k)
...
def main():
    ...
    parser.add_argument('-xd', '--arxiv_data', help='Path to arxiv clustering npy file prepared for 5-fold cv', default='D:\\arxiv_dataset\\arxiv_clustering_data_5cv.npy')
    ...
    parser.add_argument('-ed', '--emb_dim', type=int, default=None)
    parser.add_argument('-ad', '--attn_dim', type=int, default=None)
    ...
    if args.experiment == 1:
        ...
        arxiv_clustering_attn_model(args.arxiv_data, device, args.loss, args.max_num_tokens, args.max_grad_norm,
                                      args.weight_decay, args.warmup, args.lrate, args.epochs, args.model_name,
                                    args.emb_dim, args.attn_dim, args.num_attn_head, args.output_path)
