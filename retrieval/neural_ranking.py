    ...
    for epoch in range(num_epochs):
        ...
        for i in tqdm(range(train_data_len)):
            ...
            for sec in set(sample.para_labels):
                ...
                true_labels_tensor = torch.tensor(true_labels, requires_grad=True).to(device)
                rk_loss = mse(pred_score, true_labels_tensor)
                cl_loss = get_weighted_adj_rand_loss(ma, mc, true_labels, device, False) / len(true_labels)
                loss = lambda_val * rk_loss + (1.0 - lambda_val) * cl_loss
                loss.backward()
